...
__global__ void matrixMultiplicationSquare(float* M, float* N, float* P, int Width) {
  int tx = threadIdx.x;
  int ty = threadIdx.y;
  int bx = blockIdx.x;
  int by = blockIdx.y;
...
__global__ void matrixMultiplicationRectangular(float* M, float* N, float* P, int WidthM, int HeightM, int WidthN, int HeightN) {
  int tx = threadIdx.x;
  int ty = threadIdx.y;
  int bx = blockIdx.x;
  int by = blockIdx.y;
...
int main() {
  ...
  if (WidthM == HeightM && WidthN == HeightN) {
    ...
    matrixMultiplicationSquare<<<gridDim, blockDim>>>(d_M, d_N, d_P, WidthM);
    ...
    matrixMultiplicationRectangular<<<gridDim, blockDim>>>(d_M, d_N, d_P, WidthM, HeightM, WidthN, HeightN);
